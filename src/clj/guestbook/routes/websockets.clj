(ns guestbook.routes.websockets
  (:require [clojure.tools.logging :as log]
            ...
            [guestbook.auth.ws :refer [authorised?]]))
...
(defmethod handle-message :default
  ...
  {:error (str "Unrecognised websocket event type: " (pr-str id))
   :id    id})
...
(defn receive-message! [{:keys [id ?reply-fn ring-req]
  ...
  (case id
    ...
    (let [reply-fn (or ?reply-fn (fn [_]))
      ...
      (if (authorised? auth/roles message)
        (when-some [response (handle-message message)]
          (reply-fn response))
        (do
          (log/info "Unauthorised message: " id)
          (reply-fn {:message "You are not authorised to perform this action!"
                     :errors  {:unauthorised true}}))))))
